#!/usr/bin/env node
/**
 * Google Drive MCP Server — Exposed via Streamable HTTP
 *
 ...
 * 5. Server exchanges refresh_token for access_token, calls Drive API, discards
 *
...
app.post('/oauth/register', (_req, res) => {
  res.status(201).json({
    client_id: SLUG,
    client_name: 'Google Drive MCP Server',
...
app.get('/', (req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description: 'Search, read, upload, and manage Google Drive files via MCP',
    mcpEndpoint: '/mcp',
    transport: 'streamable-http',
    tools: tools.length,
    auth: {
...
function createMCPServer(client: GDriveClient): Server {
...
app.post('/mcp', async (req, res) => {
  ...
  // Create per-session Drive client
  const client = new GDriveClient(refreshToken, GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET);
...
<head>
  ...
  <title>Google Drive Connected — AgenticLedger</title>
...
<body>
  <div class="card">
    ...
    <div class="title">Google Drive Connected Successfully!</div>
    <div class="section">
    ...
    <div class="warning">
      <strong>Keep this token secure.</strong> It grants access to your Google Drive. Store it in your local mcp-keys/.env file.
    </div>
    <div class="section">
    ...
    <div class="section">
      <p><strong>For MyAgent gateway:</strong> Save to your agent's mcp-keys/googledrive.env:</p>
      <div class="config-pre">GDRIVE_REFRESH_TOKEN={{REFRESH_TOKEN}}</div>
    </div>
  </div>
  <script>
    var token = document.getElementById('refreshToken').textContent;
    document.getElementById('configBlock').textContent = JSON.stringify({mcpServers:{googledrive:{url:"${SERVER_BASE_URL}/mcp",headers:{Authorization:"Bearer "+token}}}},null,2);
...
<head>
  <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Google Drive MCP Server — AgenticLedger</title>
  <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
  <style>
    :root{--primary:#2563EB;--primary-dark:#1D4ED8;--primary-light:#DBEAFE;--primary-50:#EFF6FF;--fg:#0F172A;--muted:#64748B;--surface:#F8FAFC;--border:#E2E8F0;--success:#10B981;--success-light:#D1FAE5;--google:#4285F4;}
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .connect-btn:hover{background:#3367D6;transform:translateY(-1px);box-shadow:0 4px 12px rgba(66,133,244,.3);}
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Google Drive MCP</span></div>
    ...
    <a href="/authorize" class="connect-btn">Connect Your Google Drive</a>
    <div class="section-title">How it works</div>
    <div class="steps">
      <ol>
        <li>Click <strong>Connect Your Google Drive</strong> above</li>
...
app.listen(PORT, () => {
  console.log(`Google Drive MCP HTTP Server running on port ${PORT}`);
